...
<form action="<?= base_url('investigasi-sederhana/update/'.$inv['id']) ?>" method="POST">
<?= csrf_field() ?>
    ...
    <div class="card-body p-3 p-md-4">
        ...
        <div class="mb-4">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control form-control-sm"
                    name="<?= $name ?>" rows="3"
                    placeholder="<?= $item['ph'] ?>"><?= esc($inv[$name] ?? '') ?></textarea>
            </div>
            <?php endforeach; ?>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered align-middle mb-0" id="tbl-rekomendasi-desk">
                    ...
                    <tbody id="tbody-rekomendasi">
                        ...
                        <?php else: ?>
                        <?php foreach ($inv['rekomendasi'] as $index => $rek): ?>
                        <tr>
                            <td><textarea class="form-control form-control-sm border-0 shadow-none" name="rekomendasi[]" rows="2" placeholder="<?= ($index+1) ?>. Rekomendasi..."><?= esc($rek['rekomendasi'] ?? '') ?></textarea></td>
                            <td><input type="text" class="form-control form-control-sm border-0 shadow-none" name="pj_rekomendasi[]" placeholder="Nama penanggung jawab" value="<?= esc($rek['pj_rekomendasi'] ?? '') ?>"></td>
                            <td><input type="date" class="form-control form-control-sm border-0 shadow-none" name="tgl_rekomendasi[]" value="<?= esc($rek['tgl_rekomendasi'] ?? '') ?>"></td>
                            <td class="text-center d-print-none">
                                <button type="button" class="btn btn-outline-danger btn-sm btn-hapus-row" title="Hapus baris" <?= count($inv['rekomendasi']) <= 1 ? 'disabled' : '' ?>>
                                    <i class="bi bi-trash"></i>
                                </button>
                            </td>
                        </tr>
        ...
        <div class="d-md-none mb-2" id="mobile-rekomendasi">
            ...
            <div class="card border mb-2 mobile-rekomen-card">
                <div class="card-body p-2">
                    ...
                    <input type="date" class="form-control form-control-sm" name="tgl_rekomendasi[]">
                </div>
            </div>
            <?php else: ?>
            <?php foreach ($inv['rekomendasi'] as $index => $rek): ?>
            <div class="card border mb-2 mobile-rekomen-card">
                <div class="card-body p-2">
                    <div class="d-flex justify-content-between align-items-center mb-1">
                        <span class="fw-semibold small">Rekomendasi ke-<?= $index + 1 ?></span>
                        <button type="button" class="btn btn-outline-danger btn-sm btn-hapus-card" title="Hapus" <?= count($inv['rekomendasi']) <= 1 ? 'disabled' : '' ?>>
                            <i class="bi bi-trash"></i>
                        </button>
                    </div>
                    <label class="form-label small fw-semibold">Rekomendasi</label>
                    <textarea class="form-control form-control-sm mb-2" name="rekomendasi[]" rows="2" placeholder="<?= ($index+1) ?>. Rekomendasi..."><?= esc($rek['rekomendasi'] ?? '') ?></textarea>
                    <label class="form-label small fw-semibold">Penanggung Jawab</label>
                    <input type="text" class="form-control form-control-sm mb-2" name="pj_rekomendasi[]" placeholder="Nama penanggung jawab" value="<?= esc($rek['pj_rekomendasi'] ?? '') ?>">
                    <label class="form-label small fw-semibold">Tanggal</label>
                    <input type="date" class="form-control form-control-sm" name="tgl_rekomendasi[]" value="<?= esc($rek['tgl_rekomendasi'] ?? '') ?>">
                </div>
            </div>
            <?php endforeach; ?>
            <?php endif; ?>
            ...
            <div class="table-responsive">
                <table class="table table-bordered align-middle mb-0" id="tbl-tindakan-desk">
                    ...
                    <tbody id="tbody-tindakan">
                        ...
                        <?php else: ?>
                        <?php foreach ($inv['tindakan'] as $index => $tind): ?>
                        <tr>
                            <td><textarea class="form-control form-control-sm border-0 shadow-none" name="tindakan[]" rows="2" placeholder="<?= ($index+1) ?>. Tindakan..."><?= esc($tind['tindakan'] ?? '') ?></textarea></td>
                            <td><input type="text" class="form-control form-control-sm border-0 shadow-none" name="pj_tindakan[]" placeholder="Nama penanggung jawab" value="<?= esc($tind['pj_tindakan'] ?? '') ?>"></td>
                            <td><input type="date" class="form-control form-control-sm border-0 shadow-none" name="tgl_tindakan[]" value="<?= esc($tind['tgl_tindakan'] ?? '') ?>"></td>
                            <td class="text-center d-print-none">
                                <button type="button" class="btn btn-outline-danger btn-sm btn-hapus-row" title="Hapus baris" <?= count($inv['tindakan']) <= 1 ? 'disabled' : '' ?>>
                                    <i class="bi bi-trash"></i>
                                </button>
                            </td>
                        </tr>
        ...
        <div class="d-md-none mb-2" id="mobile-tindakan">
            ...
            <div class="card border mb-2 mobile-tindakan-card">
                <div class="card-body p-2">
                    <div class="d-flex justify-content-between align-items-center mb-1">
                        <span class="fw-semibold small">Tindakan ke-<?= $index + 1 ?></span>
                        <button type="button" class="btn btn-outline-danger btn-sm btn-hapus-card" title="Hapus" <?= count($inv['tindakan']) <= 1 ? 'disabled' : '' ?>>
                            <i class="bi bi-trash"></i>
                        </button>
                    </div>
                    <label class="form-label small fw-semibold">Tindakan yang Akan Dilakukan</label>
                    <textarea class="form-control form-control-sm mb-2" name="tindakan[]" rows="2" placeholder="<?= ($index+1) ?>. Tindakan..."><?= esc($tind['tindakan'] ?? '') ?></textarea>
                    <label class="form-label small fw-semibold">Penanggung Jawab</label>
                    <input type="text" class="form-control form-control-sm mb-2" name="pj_tindakan[]" placeholder="Nama penanggung jawab" value="<?= esc($tind['pj_tindakan'] ?? '') ?>">
                    <label class="form-label small fw-semibold">Tanggal</label>
                    <input type="date" class="form-control form-control-sm" name="tgl_tindakan[]" value="<?= esc($tind['tgl_tindakan'] ?? '') ?>">
                </div>
            </div>
            <?php endforeach; ?>
            <?php endif; ?>
        ...
        <div class="row g-3 mb-4">
            <div class="col-12 col-md-6">
                ...
                <input type="text" class="form-control form-control-sm"
                    name="manajer_nama" placeholder="Nama manajer / kepala bagian" value="<?= esc($inv['manajer_nama'] ?? '') ?>">
            </div>
            <div class="col-12 col-md-6">
                <label class="form-label fw-semibold small mb-1">Tanda Tangan</label>
                <input type="text" class="form-control form-control-sm"
                    name="ttd_manajer" placeholder="(tanda tangan)" value="<?= esc($inv['ttd_manajer'] ?? '') ?>">
            </div>
            <div class="col-12 col-md-6">
                <label class="form-label fw-semibold small mb-1">Tanggal mulai Investigasi</label>
                <input type="date" class="form-control form-control-sm"
                    name="tgl_mulai_investigasi" value="<?= esc($inv['tgl_mulai_investigasi'] ?? '') ?>">
            </div>
            <div class="col-12 col-md-6">
                <label class="form-label fw-semibold small mb-1">Tanggal selesai Investigasi</label>
                <input type="date" class="form-control form-control-sm"
                    name="tgl_selesai_investigasi" value="<?= esc($inv['tgl_selesai_investigasi'] ?? '') ?>">
            </div>
        </div>
        ...
        <div class="card border border-dark mb-0">
            ...
            <div class="card-body p-3">
                ...
                <div class="d-flex flex-column flex-sm-row align-items-start align-items-sm-center flex-wrap gap-2 py-2 border-bottom">
                    ...
                    <div class="d-flex gap-3">
                        <div class="form-check mb-0">
                            <input class="form-check-input" type="radio" name="investigasi_lengkap" value="ya" id="il_ya" <?= ($inv['investigasi_lengkap'] ?? '') == 'ya' ? 'checked' : '' ?>>
                            <label class="form-check-label fw-semibold" for="il_ya">YA</label>
                        </div>
                        <div class="form-check mb-0">
                            <input class="form-check-input" type="radio" name="investigasi_lengkap" value="tidak" id="il_tidak" <?= ($inv['investigasi_lengkap'] ?? '') == 'tidak' ? 'checked' : '' ?>>
                            <label class="form-check-label fw-semibold" for="il_tidak">TIDAK</label>
                        </div>
                    </div>
                    <span class="fw-semibold">Tanggal :</span>
                    <input type="date" class="form-control form-control-sm" style="max-width:180px;" name="tgl_investigasi_lengkap" value="<?= esc($inv['tgl_investigasi_lengkap'] ?? '') ?>">
                </div>
                ...
                <div class="d-flex flex-column flex-sm-row align-items-start align-items-sm-center flex-wrap gap-2 py-2 border-bottom">
                    ...
                    <div class="d-flex gap-3">
                        <div class="form-check mb-0">
                            <input class="form-check-input" type="radio" name="investigasi_lanjut" value="ya" id="inv_lanjut_ya" <?= ($inv['investigasi_lanjut'] ?? '') == 'ya' ? 'checked' : '' ?>>
                            <label class="form-check-label fw-semibold" for="inv_lanjut_ya">YA</label>
                        </div>
                        <div class="form-check mb-0">
                            <input class="form-check-input" type="radio" name="investigasi_lanjut" value="tidak" id="inv_lanjut_tidak" <?= ($inv['investigasi_lanjut'] ?? '') == 'tidak' ? 'checked' : '' ?>>
                ...
                <div class="d-flex flex-column flex-sm-row align-items-start align-items-sm-center flex-wrap gap-2 pt-2">
                    ...
                    <div class="d-flex gap-3 flex-wrap">
                        <div class="form-check mb-0">
                            <input class="form-check-input" type="radio" name="grading_ulang" value="hijau" id="gu_hijau" <?= ($inv['grading_ulang'] ?? '') == 'hijau' ? 'checked' : '' ?>>
                            <label class="form-check-label fw-bold text-success" for="gu_hijau">Hijau</label>
                        </div>
                        <div class="form-check mb-0">
                            <input class="form-check-input" type="radio" name="grading_ulang" value="kuning" id="gu_kuning" <?= ($inv['grading_ulang'] ?? '') == 'kuning' ? 'checked' : '' ?>>
                            <label class="form-check-label fw-bold text-warning" for="gu_kuning">Kuning</label>
                        </div>
                        <div class="form-check mb-0">
                            <input class="form-check-input" type="radio" name="grading_ulang" value="merah" id="gu_merah" <?= ($inv['grading_ulang'] ?? '') == 'merah' ? 'checked' : '' ?>>
