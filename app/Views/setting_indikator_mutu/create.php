    ...
    <div class="col-md-6">
        <div class="card card-primary card-outline mb-4">
            <div class="card-header">
                <h3 class="card-title">Tambah Setting Indikator Mutu</h3>
            </div>
            <form action="<?= base_url('setting-indikator-mutu/store') ?>" method="post">
                <div class="card-body">
                        ...
                        <div class="col-sm-8">
                            <select class="form-select select2" id="indikator_mutu_id" name="indikator_mutu_id" required>
                                ...
                                <?php foreach ($indikator_mutu as $indikator) : ?>
                                    <option value="<?= $indikator['id'] ?>" <?= old('indikator_mutu_id') == $indikator['id'] ? 'selected' : '' ?>><?= esc($indikator['judul_indikator']) ?></option>
                                <?php endforeach ?>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-8">
                            <select class="form-select select2" id="area_pengukuran_id" name="area_pengukuran_id[]" multiple required>
                                <?php foreach ($area_pengukuran as $area) : ?>
                                    <option value="<?= $area['id'] ?>" <?= in_array($area['id'], old('area_pengukuran_id', [])) ? 'selected' : '' ?>><?= esc($area['area_pengukuran']) ?></option>
                                <?php endforeach ?>
                            </select>
